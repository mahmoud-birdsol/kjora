        ...
        <div class="relative mt-1">
            <button type="button"
                ...
                <span class="flex items-center cursor-pointer" v-if="selected">
                    <img :src="selected[imageName]" alt="" class="h-6 w-6 flex-shrink-0 rounded-full cursor-pointer" @click="showDropDown = ! showDropDown">
                    <input type="text" :value="modelValue" @input="$emit('update:modelValue', $event.target.value)" class="block w-full px-4 mx-4 py-0 my-0 border-none sm:text-sm disabled:bg-gray-100 focus:ring-none focus:border-none ring-transparent">
                </span>
                <span class="cursor-pointer absolute inset-y-0 right-0 ml-3 flex items-center pr-2" @click="showDropDown = ! showDropDown">
