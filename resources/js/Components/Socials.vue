...
<template>
   <onClickOutside @trigger="showSocials = false">
      ...
      >
         <div
            v-if="showSocials"
            class="absolute z-30 flex flex-col gap-3 p-3 text-xs text-white bg-black border rounded-lg ltr:-right-1 rtl:-left-1 border-neutral-500"
            :class="position"
         >
            <a
               :data-href="
            ...
            <a
               :href="
                  'https://twitter.com/intent/tweet?hashtags=kjora&text=' + url
               "
               target="_blank"
               class="relative flex items-center gap-2 [&>div]:hover:block"
               @click="showSocials = false"
            >
               ...
               <span class="whitespace-nowrapp">{{
                  $t("share-to-twitter")
               }}</span>
            </a>
            <div
               class="flex items-center gap-2 [&>div]:hover:block cursor-pointer"
               @click="copy"
            >
