                    ...
                    <RadioGroup v-model="form.report_option_id"
                        ...
                        <template v-for="option in options">
                            <RadioGroupOption v-slot="{ checked }" :value="option.id">
                                <li class="text-black px-4 min-h-[60px]  flex items-center justify-center text-sm"
                                    :class="checked ? 'border-primary text-primary ' : 'border-black'">
                                    <div class="text-justify first-letter:capitalize">{{ option.body[$page.props.locale] }}</div>
                                </li>
                            </RadioGroupOption>
                        </template>
                    </RadioGroup>
