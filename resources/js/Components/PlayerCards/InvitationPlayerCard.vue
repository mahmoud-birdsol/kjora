...
//invitation states
const isHiring = userStore.isCurrentUser(props.invitation.inviting_player);
const isPending =
   props.invitation.state === null &&
      ...
      <div class="flex flex-col items-center justify-center gap-3 p-4">
         <div class="flex flex-col items-center">
            ...
            <p class="flex items-center justify-center space-x-2 text-sm">
               <span
                  ...
                  :class="
                     txtColor == 'black' ? 'text-primary' : 'text-[#FF9900]'
                  "
               >
         ...
         <div class="flex flex-col items-center">
            ...
            <template v-if="!isHiring">
               <p class="text-center text-[10px] text-stone-300/70">
                  <span v-if="invitation.state === 'declined'">{{
                     $t("declined")
                  }}</span>
                  <span v-if="isCancelled"> {{ $t("canceled") }}</span>
                  <span v-if="isPending">
                     {{ $t("wants-to-invite-you-for") }}</span
                  >
                  {{ $t("match-in") }}
                  <DateTranslation
                     format="DD MMMM YYYY, hh:mm A"
         ...
         <div class="self-stretch text-sm font-bold">
            ...
            <Link
               :href="
                  route('chats.by.user.show', {
                     user: isHiring
                        ? invitation.invited_player_id
                        : invitation.inviting_player_id,
            ...
            <!-- Rate -->
            <Link
               :href="route('player.review.show', invitation.reviews[0].id)"
               v-if="isAccepted && isShouldRate"
               class="flex items-center justify-center w-full px-4 py-2 rounded-full shadow-sm bg-stone-100 enabled:hover:bg-opacity-90 enabled:active:scale-95"
            >
               {{ $t("rate") }}
