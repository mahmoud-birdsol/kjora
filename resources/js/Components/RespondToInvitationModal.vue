	...
	<Modal
		...
		<div class="rounded-xl bg-white min-h-[300px]">
			<div class="flex flex-col items-center justify-between px-6 pb-6">
				<div class="w-full">
					<h2 class="text-xl font-bold text-center uppercase text-primary">
						{{ $t('invitation') }}
					</h2>
					<p class="mt-6 text-sm font-light text-gray-700">
						{{
							$t(
								'you-have-received-an-invitation-from- :name ,-to-play-a-game-on',
								{ name: invitation.inviting_player.name }
							)
						}}
						<strong>
							<DateTranslation
								format="DD MMMM YYYY, h:m A"
								:start="invitation.date" /> </strong
						><span> {{ $t('at') }} </span>
						<strong
