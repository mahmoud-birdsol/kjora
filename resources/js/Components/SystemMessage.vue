...
<template>
	<div
		...
		v-if="message && showSystemMessage"></div>
	<FadeInTransition>
		<div
			v-if="message && showSystemMessage"
			class="fixed bottom-0 z-40 flex justify-between w-full max-md:text-xs"
			id="SysMessage">
			<!-- Info Message -->
			<div
				v-if="message.type === 'info'"
				class="flex w-full px-6 py-4 bg-sky-500">
				<div class="flex items-center justify-between w-full mt-3 space-x-4">
					<p class="mr-16 font-bold text-sky-50">
						{{ message.body }}
					</p>
					<Link
						v-if="message.action"
				...
				<div
					...
					<Link
						...
						<SecondaryButton
							...
							>{{ $t(message.action.text) }}</SecondaryButton
						>
					</Link>
			...
			<div
				...
				class="flex w-full px-6 py-4 bg-rose-500">
				<div class="flex items-center justify-between w-full mt-3 space-x-4">
					<p class="mr-16 font-bold text-rose-50">
						{{ $t(message.body) }}
					</p>
					<Link
						v-if="message.action"
			...
			<div
				...
				class="flex w-full px-6 py-4 bg-emerald-500">
				<div class="flex items-center justify-between w-full mt-3 space-x-4">
					<p class="mr-16 font-bold text-emerald-50">
						{{ message.body }}
					</p>
					<Link
						v-if="message.action"
