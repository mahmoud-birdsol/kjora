...
function ready() {
	if (
		cropper.cropped &&
		cropper.ready &&
		cropper.originalUrl == imageObj.value.url
	) {
...
<template>
	<div class="overflow-hidden">
		<SlideDownTransition>
			<div
				v-if="open"
				class="flex flex-col gap-4 p-6 bg-white border-t min-w-7xl">
				...
				<div class="flex justify-center max-w-xs gap-4 mx-auto">
					<PrimaryButton @click="$emit('update:open')"> Cancel </PrimaryButton>
					<PrimaryButton
						@click="getResult"
						class="self-center !px-6">
						Crop
					</PrimaryButton>
				</div>
			</div>
