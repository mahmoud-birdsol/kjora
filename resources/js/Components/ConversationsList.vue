...
const debouncedSubmitSearchFriends = window._.debounce(submitSearchFriends, 300)
function submitSearchFriends(e) {
    filteredConversations.value = props.conversations.filter(c => c.users[0].name.includes(searchFriendsForm.query))
}
</script>
    ...
    <div class="flex flex-col h-full gap-2">
        ...
        <div class="grid items-center w-full grid-cols-1 ">
            <input type="search" v-model="searchFriendsForm.query" @input="debouncedSubmitSearchFriends"
                class="w-full col-start-1 row-start-1 focus:ring-primary focus:border-primary focus:shadow-sm focus:shadow-primary2 pis-10 rounded-3xl">
