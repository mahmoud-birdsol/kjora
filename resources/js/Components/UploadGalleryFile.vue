...
const uploadFiles = async () => {
	...
	if (!postId) {
		try {
			...
			console.log(
				'there is error uploading this file (cover) ' + filesData.value[0].name
			)
				...
				<div
					...
					<div class="relative grid grid-cols-3 gap-2">
						...
						<FadeInTransition>
							<div
								v-if="isLoading"
								class="absolute inset-0 z-20 grid w-full h-full p-4 bg-stone-400/50 place-content-center"></div>
						</FadeInTransition>
					</div>
				</div>
			</div>
