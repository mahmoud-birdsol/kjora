...
const uploadFiles = async () => {
    ...
    if (!postId) {
        try {
            ...
            console.log(
                "there is error uploading this file (cover) " +
                    filesData.value[0].name
            );
    ...
    const promises = filesData.value.slice(1).map((fileData, i) => {
        ...
        if (file.type.startsWith("image") || file.type.startsWith("video")) {
            return axios
                ...
                .catch((error) => {
                    console.log(
                        "there is error uploading this file " + file.name
                    );
        ...
        <div class="flex flex-col min-h-[500px] justify-between p-6 pt-0">
            ...
            <div>
                ...
                >
                    {{
                        $t(
                            "only videos and images with max size (84MB) are allowed"
                        )
                    }}
                </div>
                <PrimaryButton
