                ...
                <div v-show="showPreview" class="relative overflow-auto hideScrollBar max-h-80" v-loading="isLoading">
                    <div class="relative grid grid-cols-3 gap-2">
                        <template v-for="(file, index) in filesData" :key="index">
                            <div v-if="file.url.startsWith('data:image') ||
                                ...
                                <button @click.prevent.stop="removePhoto(index)" class="absolute top-0 right-0 bg-white rounded-bl-xl bg-opacity-90">
                                    <div class="flex flex-col items-start justify-center h-full p-1 opacity-100">
                                        <XMarkIcon class="w-5 h-5 text-stone-800" />
                                    </div>
                                </button>
                                <button class="absolute top-0 left-0 bg-white rounded-br-xl bg-opacity-90" @click.stop="showCropModal(file)" v-if="file.url.startsWith('data:image') ||
                                    file.type.startsWith('image')
                                    ">
                                    <div class="flex flex-col items-start justify-center h-full p-1 opacity-100">
                                        <CropIcon class="w-4" />
                            ...
                            <div v-else-if="file.url.startsWith(
                                ...
                                <button @click.prevent="removePhoto(index)" class="absolute top-0 right-0 bg-white rounded-bl-xl bg-opacity-90">
                                    <div class="flex flex-col items-start justify-center h-full p-1 opacity-100">
                                        <XMarkIcon class="w-5 h-5 text-stone-800" />
                                    </div>
                                </button>
                            </div>
                            <div v-else-if="file.type.endsWith('.document') ||
                        ...
                        <FadeInTransition>
                            <div v-if="isLoading" class="absolute inset-0 z-20 grid w-full h-full p-4 place-content-center bg-stone-400/50">
                            </div>
                        </FadeInTransition>
                    </div>
                </div>
