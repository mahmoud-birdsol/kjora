...
<template>
    <div :class="isCurrentUser ? 'text-white' : 'text-stone-800'" class="w-full h-full overflow-hidden rounded-md">
        ...
        <div v-else-if="media?.mime_type.endsWith('.document') || media?.mime_type.startsWith('application/msword') || media?.mime_type.startsWith('application/pdf')" class="flex items-center justify-between gap-2">
            <div class="flex gap-1 ">
                <img v-if="media?.mime_type.endsWith('.document') || media?.mime_type.startsWith('application/msword')" class="object-contain w-7 h-7" src="/images/doc.png" />
                <img v-if="media?.mime_type.startsWith('application/pdf')" class="object-contain w-7 h-7" src="/images/pdf.png" />
