	...
	<FadeInTransition appear>
		<div
			...
			<div class="max-w-[60%]">
				<div
					:class="[bodyClass]"
					class="max-w-full p-4 rounded-2xl">
					<!-- replied message -->
					<div
						...
						<div
							...
							:class="isCurrentUser ? 'text-primary' : 'text-white'">
								...
								<div
									...
									{{
										imagesVideosOnly.length > 3
											? `+ ${imagesVideosOnly.length - 3} ${
													$page.props.locale == 'en' ? 'more...' : 'المزيد ...'
												}`
											: `${
													$page.props.locale == 'en' ? 'show all' : 'عرض الجميع'
												}`
							...
							<ul class="flex flex-col justify-center gap-y-2">
								<button
									...
									<ConfirmationModal
										...
										<template #body>
											<span>{{
												$t('Are you sure you want delete this message ? ')
											}}</span>
										</template>
									</ConfirmationModal>
								</button>
			...
			<button
				...
				<ReplyIcon
					class="fill-transparent hover:fill-black stroke-black"></ReplyIcon>
			</button>
		</div>
	</FadeInTransition>
</template>
