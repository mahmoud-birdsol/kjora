...
<template>
   <div>
      <p v-show="!isEditingCaption" class="text-sm text-stone-500">
         {{ captionForm.caption }}
      </p>
      <div v-show="isEditingCaption" class="flex flex-col justify-end gap-3">
         ...
         <div class="flex justify-end gap-2">
            <PrimaryButton @click="cancelEditCaption" class="w-24">{{
               $t("Cancel")
            }}</PrimaryButton>
            <PrimaryButton @click="submitEditCaption" class="w-24">{{
               $t("Save")
            }}</PrimaryButton>
         </div>
      </div>
