...
const props = defineProps(["postMedia"]);
...
<template>
   <div class="relative">
      <Splide
         ...
         :options="options"
      >
         <template v-for="media in postStore.post.media" :key="media.id">
            <SplideSlide class="">
      ...
      <div
         v-if="postStore.PostMediaCount > 1"
         class="absolute p-1 text-xs top-2 right-2 text-stone-200"
      >
         <span>{{ currentMediaIndex }}</span>
         <span>/</span>
         <span>{{ postStore.PostMediaCount }} </span>
      </div>
   </div>
</template>
