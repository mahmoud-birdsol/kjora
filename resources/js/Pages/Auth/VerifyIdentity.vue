...
onMounted(() => {
   if (userStore.currentUser.identity_front_image_url != null) {
      identityFrontImagePreview.value =
         userStore.currentUser.identity_front_image_url;
   }

   if (userStore.currentUser.identity_back_image_url != null) {
      identityBackImagePreview.value =
         userStore.currentUser.identity_back_image_url;
   }

   if (userStore.currentUser.identity_selfie_image_url != null) {
      identitySelfieImagePreview.value =
         userStore.currentUser.identity_selfie_image_url;
   }
});
...
const form = useForm({
   identity_issue_country:
      userStore.currentUser.identity_issue_country ?? "Kuwait",
   identity_type: userStore.currentUser.identity_type,
   identity_front_image: userStore.currentUser.identity_front_image_url,
   identity_back_image: userStore.currentUser.identity_back_image_url,
   identity_selfie_image: userStore.currentUser.identity_selfie_image_url,
});
let identity_status = userStore.currentUser.identity_status;
            ...
            <CardContent :title="$t('verify identity')">
               <template #body>
                  <div class="flex flex-col justify-between h-full">
                     <div class="min-h-[200px]">
                        <div>
                           <h3 class="mb-4 text-lg font-bold text-gray-900">
                              {{ $t("use a valid government-issued document") }}
                           </h3>
                        ...
                        <div class="mt-4">
                           ...
                           <InputError
                              class="mt-2"
                              :message="form.errors.identity_issue_country"
                           />
                        </div>
                     </div>
                     ...
                     <div>
                        <div
                           class="grid grid-cols-1 gap-4 mt-12 sm:grid-cols-2"
                        >
                           <MegaButton
                        ...
                        <InputError
                           class="mt-2"
                           :message="form.errors.identity_type"
                        />
                     </div>
                  </div>
               </template>
            ...
            <CardContent :title="$t('upload identity')">
               <template #body>
                  <div class="flex flex-col justify-between h-full gap-2">
                     <div class="min-h-[200px]">
                        <div>
                           <h3 class="mb-8 font-bold text-black uppercase">
                              {{ $t("take-selfie-photo") }}
                           </h3>
                        </div>
                        <div>
                           <div class="flex gap-4 max-md:flex-col">
                              <div class="md:w-1/4">
                                 <button
                                    @click="showIdentitySelfieModal = true"
                                    class="relative w-full h-full group"
                                 >
                                    <img
                                       v-if="form.identity_selfie_image"
                                       :src="form.identity_selfie_image"
                                       alt=""
                                       class="w-full h-auto"
                                    />
                           ...
                           <InputError
                              class="mt-2"
                              :message="form.errors.identity_selfie_image"
                           />
                        </div>
                        <div>
                     ...
                     <div class="grid grid-cols-1 gap-4 mt-10 sm:grid-cols-2">
                        <div>
                           ...
                           <InputError
                              class="mt-2"
                              :message="form.errors.identity_front_image"
                           />
                        </div>
                        ...
                        <div>
                           ...
                           <InputError
                              class="mt-2"
                              :message="form.errors.identity_back_image"
                           />
                        </div>
                     </div>
                  </div>
               ...
               <template #footer>
                  <div class="mt-4">
                     <PrimaryButton
                        v-show="!(identity_status === 'Verified')"
