    ...
    <div class="rounded-lg bg-gray-100 p-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <div class="max-w-sm">
                    <MainPlayerCard
                        :player="invitation.inviting_player"
                        :show-report="true"
                        :show-invite="false"
                        :show-location="true"
                    />
            ...
            <div class="flex flex-col justify-between items-start">
                ...
                <button v-if="invitation.state == 'declined'" :disabled="true"
                        class="w-full flex justify-center items-center px-4 py-2 bg-rose-300 bg-opacity-50 border border-transparent rounded-full font-bold text-xs text-black text-center uppercase tracking-widest focus:outline-none focus:ring transition">
                    Declined
                </button>

                <PrimaryButton
                    v-if="invitation.state == null"
                    @click="showInvitationNotificationModal = true"
                >Respond
                </PrimaryButton>
            </div>
        </div>
