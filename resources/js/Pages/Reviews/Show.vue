...
const labels = props.ratingCategories.map(
    (r) => r.name[usePage().props.locale]
);
...
<template>
    ...
    <AppLayout :title="$t('rate')" :showBall="false">
        ...
        <div
            class="w-full h-full py-6 bg-white sm:p-6 rounded-xl md:w-1/2 md:mx-auto"
        >
            <div
                class="flex flex-col items-center justify-between gap-2 mb-4 text-black"
            >
                <div class="flex flex-col gap-3">
                    <div class="relative mx-auto min-w-max">
                        <Avatar
                            :id="review.player.id"
                            :username="review.player.name"
                            :image-url="review.player.avatar_url"
                            :size="'xlg'"
                            :border="true"
                            border-color="primary"
                        />
                        <Link
                            :href="route('player.profile', review.player.id)"
                    ...
                    <div class="flex flex-col gap-0 text-center">
                        <Link :href="route('player.profile', review.player.id)">
                            <h3
                                class="text-lg font-bold capitalize text-primary"
                            >
                                {{ review.player.name }}
                            </h3>
                        </Link>
                        <Link :href="route('player.profile', review.player.id)"
                            >@{{ review.player.username }}</Link
                        >
                        <div
                            class="flex flex-wrap gap-2 text-xs text-stone-400"
                        >
                            <span
                                >{{ $t("age") }}: {{ review.player.age }}</span
                            >
                            <span
                                >{{ $t("played") }}:
                                {{ review.player.played }}</span
                            >
                            <span
                                >{{ $t("missed") }}:
                                {{ review.player.missed }}</span
                            >
                            <span
                                >{{ $t("Position") }}:
                                {{ review.player.position.name[locale] }}</span
                            >
                        </div>
                    </div>
                </div>
                <div class="flex justify-center gap-4 item-center">
                    <span
                        class="flex items-center scale-90 ltr:origin-right rtl:origin-left gap-x-1 text-gold"
                    >
                        <span class="flex items-center gap-1">
                            <template v-for="i in 5">
                                <StarIconFilled
                                    class="w-5 h-5"
                                    v-if="review.player.rating >= i"
                                />
                    ...
                    <span class="flex items-center text-xs text-stone-400"
                        >{{
                            $t("based on :count players", {
                                count: review.player.played,
                            })
                        }}
                    </span>
                </div>
            ...
            <div class="flex flex-col mt-6 overflow-hidden gap-y-8">
                <div
                    class="flex flex-col justify-between h-full max-sm:mb-10 md:justify-self-end"
                >
                    <RatingChart
                        :data="graphData"
                        :labels="labels"
                        theme="rgb(0,100,0)"
                        overlay="rgba(0,100,0,0.2)"
                    />
                </div>
                <form class="flex flex-col gap-4 px-5">
                    ...
                    <template v-if="ratingForm.attended">
                        <template
                            v-for="item in ratingCategories"
                            :key="item.id"
                        >
                            <div class="flex flex-col gap-2">
                                <div class="flex justify-between">
                                    ...
                                    <span
                                        ...
                                        >{{
                                            ratingForm.ratingCategory.filter(
                                                (c) => c.id === item.id
                                            )[0].value
                                        }}</span
                                    >
                                ...
                                <div
                                    class="px-6 py-1 mx-4 border border-white rounded-full bg-stone-300"
                                >
                                    <el-slider
                                        v-model="
                                            ratingForm.ratingCategory.filter(
                ...
                <button
                    ...
                    class="p-3 px-6 m-4 mt-4 text-white bg-black rounded-full"
                >
                    {{ $t("rate") }}
                </button>
            </div>
        ...
        >
            ...
            <div class="bg-white rounded-xl p-6 md:min-h-[300px]">
                <div
                    class="flex flex-col justify-between items-center h-56 md:min-h-[200px]"
                >
                    ...
                    <div class="text-center">
                        ...
                        <p class="">
                            {{
                                $t(
                                    "we appreciate you taking the time to share your rating"
                                )
                            }}.
                        </p>
                    </div>
