...
watch(
    ...
    (newValue) => {
        ...
        newValue > 0
            ? (showNotificationIndicator.value = true)
...
function markAllNotificationsAsRead() {
    ...
    notifications.value.forEach((n, i) => {
        axios
            .post(route("api.notifications.mark-as-read", n.id))
            .then((res) => {
...
<template>
    <nav class="bg-transparent">
        ...
        <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="flex items-center h-16 gap-1 md:justify-between">
                <div class="hidden md:flex">
                    <!-- Navigation Links -->
                    <div
                        class="items-center hidden gap-2 lg:gap-8 sm:-my-px md:flex"
                    >
                ...
                <div
                    class="flex items-center md:gap-x-3 max-md:justify-between max-md:w-full"
                >
                    <button
                    ...
                    <div class="flex items-center justify-between gap-2">
                        ...
                        >
                            ...
                            <Link
                                class="uppercase max-xs:text-xs"
                                :href="route('upgrade')"
                                >{{ $t("subscribe") }}</Link
                            >
                        </button>
                        <!-- user city  -->
                        <div
                            class="flex items-center gap-1 rtl:flex-row-reverse"
                        >
                            <MapPinIcon class="w-4 h-4 text-primary" />
                            <span
                                class="text-white max-lg:w-min max-xs:text-xs"
                                >{{ currentUser.current_city }}</span
                            >
                        </div>
                        <!-- Settings Dropdown -->
                        <div class="relative">
                        ...
                        <div class="relative">
                            ...
                            >
                                ...
                                <template #content>
                                    ...
                                    <div
                                        class="block px-4 py-2 text-xs text-center"
                                    >
                                        <Link
        ...
        <!-- Responsive Navigation Menu -->
        <div
            class="fixed top-0 bottom-0 left-0 right-0 z-40 bg-black bg-opacity-50 md:hidden"
            :class="{
        ...
        <SlideInTransition>
            <div
                class="pt-2 pb-3 space-y-1 fixed top-0 bg-black h-full w-[max(20em,50%)] z-50"
                v-if="showingNavigationDropdown"
            >
                <div class="flex">
                    <XMarkIcon
                        class="w-5 m-3 text-white mis-auto"
                        @click="showingNavigationDropdown = false"
                    />
                </div>
